...
class ProxyActivity : BaseActivity<ProxyDesign>() {
    override suspend fun main() {
        val mode = withClash {
            ...
            session.mode ?: queryOverride(Clash.OverrideSlot.Persist).mode
        }
                    ...
                    when (it) {
                        ...
                        is ProxyDesign.Request.PatchMode -> {
                            ...
                            withClash {
                                ...
                                val persistOverride = queryOverride(Clash.OverrideSlot.Persist)
                                val desiredPersistMode = when (it.mode) {
                                    TunnelState.Mode.Global -> it.mode
                                    else -> null
                                }

                                if (persistOverride.mode != desiredPersistMode) {
                                    persistOverride.mode = desiredPersistMode
